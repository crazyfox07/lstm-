...
f = open('data/data_stock.csv')
df = pd.read_csv(f)  # 读入股票数据
data = df.iloc[:, 2:].values  # 取第3到最后一列（open,close,high,low,volume,amount,return）的值，数值类型是ndarray

# normalize_data=scale(data,axis=0)#axis=0代表对每一列标准归一化,axis=1代表对每一行标准归一化，不写axis代表对整个矩阵归一化，(data-np.mean(data))/np.std(data)
min_max_scaler = MinMaxScaler()
normalize_data = min_max_scaler.fit_transform(data)  # .对每一列进行最小－最大规范化
...
train_x, train_y = [], []  # 训练集，可以将train_x转换为ndarray，查看其维度
...
def train_lstm():
    ...
    with tf.Session() as sess:
        ...
        while (True):
            ...
            # 每2000步保存一次参数
            if step % 2000 == 0:
                print("保存模型：", saver.save(sess, 'model_stock/model_stock', global_step=step))

            # 当loss_小于某个值值，保存模型，退出
            if loss_ < 0.000001:
                print("over  保存模型：", saver.save(sess, 'model_stock/model_stock', global_step=step))
                break
...
def prediction():
    ...
    with tf.Session() as sess:
        ...
        for i in range(300):
            ...
            predict.append(next_seq[-1].tolist())
        ...
        plt.figure()
        plt.plot(list(range(len(normalize_data))), normalize_data[:, 1],
                 color='b')  # normalize_data[:,1]代表股市的收盘价（close）
        plt.plot(list(range(len(normalize_data), len(normalize_data) + len(predict))), predict[:, 1], color='r')
        plt.show()
